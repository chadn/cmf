...
export class NineteenHzEventSource extends BaseEventSourceHandler {
    ...
    private parseEventRow($: cheerio.Root, $row: cheerio.Cheerio): ParsedEventRow | null {
        try {
            ...
            const links = linksCell.find('a').map((_, link) => {
            ...
            logr.warn('api-es-19hz', `Error parsing event row: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return null
        }
    }
    ...
    private createCmfEvent(parsed: ParsedEventRow): CmfEvent | null {
        try {
            ...
            const eventId = parsed.eventUrl ? 
                parsed.eventUrl.split('/').pop()?.replace(/[^a-zA-Z0-9]/g, '') || 
            ...
            logr.warn('api-es-19hz', `Error creating CmfEvent: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return null
        }
    }
}
